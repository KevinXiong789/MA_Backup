...
BODY_PARTS_KPT_IDS = ( [ 1,  2], [ 1,  5], [2,  3], [3,  4], [ 5,  6],
                       [ 6,  7], [ 1,  8], [8,  9], [9, 10], [ 1, 11],
                       [11, 12], [12, 13], [1,  0], [0, 14], [14, 16],
                       [ 0, 15], [15, 17], [2, 16], [5, 17] )
BODY_PARTS_PAF_IDS = ( [12, 13], [20, 21], [14, 15], [16, 17], [22, 23],
...
def extract_keypoints (heatmap, all_keypoints, total_keypoint_num):
    ...
    heatmap_peaks = (heatmap_center > heatmap_left) & (heatmap_center > heatmap_right) & \
                    (heatmap_center > heatmap_up) & (heatmap_center > heatmap_down)
...
def connections_nms (a_idx, b_idx, affinity_scores):
    ...
    for index, (i, j) in enumerate(zip(a_idx, b_idx) ):
...
def group_keypoints(all_keypoints_by_type, pafs, pose_entry_size=20, min_paf_score=0.05):
    ...
    for part_id in range(len(BODY_PARTS_PAF_IDS) ):
        ...
        keypoints_a = all_keypoints_by_type[BODY_PARTS_KPT_IDS[part_id][0] ]
        keypoints_b = all_keypoints_by_type[BODY_PARTS_KPT_IDS[part_id][1] ]
        n = len(keypoints_a)
        m = len(keypoints_b)
        ...
        connections = list(zip(keypoints_a[a_idx, 3].astype(np.int32),
                               keypoints_b[b_idx, 3].astype(np.int32),
                               affinity_scores))
        if len(connections) == 0:
            continue

        if part_id == 0:
            pose_entries = [np.full(shape=pose_entry_size, fill_value=-1) for _ in range(len(connections) ) ]
            ...
            for i in range(len(connections) ):
                ...
                if num == 0:
                    pose_entry = np.full(shape=pose_entry_size, fill_value=-1)
